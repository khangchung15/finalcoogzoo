...
function ManageExhibits({ exhibitData, setExhibitData, addExhibit, exhibitId, setExhibitId, deleteExhibit,showSidebar }) {
  ...
  useEffect(() => {
    const fetchExhibits = async () => {
      try {
        const response = await fetch('https://coogzootestbackend-phi.vercel.appexhibits');
        if (!response.ok) {
  ...
  const handleUpdateExhibit = async () => {
    ...
    try {
      const response = await fetch(`https://coogzootestbackend-phi.vercel.appupdate-exhibit?id=${updateData.ID}`, {
        method: 'PUT',

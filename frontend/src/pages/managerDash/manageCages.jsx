...
function ManageCages({ cageData, setCageData, addCage, cageId, setCageId }) {
  ...
  useEffect(() => {
    const fetchCages = async () => {
      ...
      try {
        const data = await fetchWithRetry('https://coogzootestbackend-phi.vercel.app/cages');
        setCages(data);
  ...
  useEffect(() => {
    const fetchExhibits = async () => {
      ...
      try {
        const data = await fetchWithRetry('https://coogzootestbackend-phi.vercel.app/exhibits');
        setExhibits(data);
  ...
  const updateCage = async () => {
    ...
    try {
      const response = await fetchWithRetry(
        `https://coogzootestbackend-phi.vercel.app/update-cage?id=${updateData.id}`,
        {
          method: 'PUT',
  ...
  const handleDeleteCage = async () => {
    try {
      await fetchWithRetry(
        `https://coogzootestbackend-phi.vercel.app/remove-cage?id=${cageId}`,
        { method: 'DELETE' }
      );

...
function ManageCages({ cageData, setCageData, addCage, cageId, setCageId }) {
  ...
  useEffect(() => {
    const fetchCages = async () => {
      ...
      try {
        const data = await fetchWithRetry('https://finalcoogzoobackend.vercel.app/cages');
        setCages(data);
  ...
  const updateCage = async () => {
    ...
    try {
      const response = await fetchWithRetry(
        `https://finalcoogzoobackend.vercel.app/update-cage?id=${updateData.id}`,
        {
          method: 'PUT',
  ...
  const handleDeleteCage = async () => {
    try {
      await fetchWithRetry(
        `https://finalcoogzoobackend.vercel.app/remove-cage?id=${cageId}`,
        { method: 'DELETE' }
      );
